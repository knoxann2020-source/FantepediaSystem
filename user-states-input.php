...
if (mysqli_num_rows($table_check_result) == 0) {
    ...
    if (!mysqli_query($connection, $create_table_query)) {
        ...
        header('location: ' . ROOT_URL . 'user-states-input.php');
        die();
    }
}

// Dynamic Fante States list from DB
$fante_states_query = "SELECT DISTINCT state_name FROM fante_states WHERE status IN ('approved', 'pending') ORDER BY state_name";
$fante_states_result = mysqli_query($connection, $fante_states_query);
$fante_states = [];
while ($row = mysqli_fetch_assoc($fante_states_result)) {
    $fante_states[] = $row['state_name'];
}
if (empty($fante_states)) {
    $fante_states = ['Mankessim', 'Abura', 'Oguaa', 'Gomoa']; // fallback
}

// Handle form submission
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $state_name = mysqli_real_escape_string($connection, $_POST['state_name']);
    ...
    $check_query = "SELECT id FROM fante_states WHERE user_id = ? AND state_name = ? AND status = 'pending'";
    $check_stmt = mysqli_prepare($connection, $check_query);
    mysqli_stmt_bind_param($check_stmt, 'is', $user_id, $state_name);
    mysqli_stmt_execute($check_stmt);
    mysqli_stmt_store_result($check_stmt);

    if (mysqli_stmt_num_rows($check_stmt) > 0) {
        $_SESSION['contribution_error'] = 'You have already submitted this state and it is pending approval.';
    } else {
        $query = "INSERT INTO fante_states (state_name, details, video, latitude, longitude, status, user_id) VALUES (?, ?, ?, ?, ?, 'pending', ?)";
        $stmt = mysqli_prepare($connection, $query);
        mysqli_stmt_bind_param($stmt, 'ssssdi', $state_name, $details, $video, $latitude, $longitude, $user_id);
        if (mysqli_stmt_execute($stmt)) {
            $_SESSION['contribution_success'] = 'Your state contribution has been submitted and is pending approval.';
        } else {
...
$user_id = $_SESSION['user-id'];
$query = "SELECT * FROM fante_states WHERE user_id = ? AND status = 'pending' ORDER BY created_at DESC";
$stmt = mysqli_prepare($connection, $query);
...
// Force Light Mode for User States Input Page
echo '<style>
    body.dark-mode {
    ...
    body.dark-mode .contribution-form select {
...
<section class="contribution">
    <div class="container contribution__container">
        <h2>Contribute to Fante States</h2>
        <p>Help expand our Fante states database by submitting information about Fante states.</p>
        ...
        <div class="contribution-form-container">
            <h3>Submit State Information</h3>
            <form action="" method="POST" enctype="multipart/form-data" class="contribution-form">
                <div class="form-group">
                    <label for="state_name">Fante State *</label>
                    <select id="state_name" name="state_name" required>
                        <option value="">Select a Fante State</option>
                        <?php foreach ($fante_states as $state): ?>
                            <option value="<?= $state ?>"><?= $state ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="form-group">
                    <label for="details">State Details *</label>
                    <textarea id="details" name="details" placeholder="Provide detailed information about the Fante state" required></textarea>
                </div>
                <div class="form-group">
                    <label for="video">Video (Optional)</label>
                    <input type="file" id="video" name="video" accept="video/*">
                    <small>Upload a video related to the state</small>
                </div>
                <div class="form-group">
                    <label>State Location (Click on map to set coordinates - Optional)</label>
                    <div id="map" style="height: 300px; width: 100%; margin-bottom: 10px; border-radius: 10px;"></div>
                    <input type="hidden" id="latitude" name="latitude">
                    <input type="hidden" id="longitude" name="longitude">
                    <small>Click on the map to set the location coordinates for this state (optional).</small>
                </div>
                ...
                <div class="contributions-list">
                    <?php while ($contribution = mysqli_fetch_assoc($contributions)) : ?>
                        <div class="contribution-item">
                            <h4><?= htmlspecialchars($contribution['state_name']) ?></h4>
...
.contribution-form input, .contribution-form textarea, .contribution-form select {
    width: 100%;
    padding: 0.75rem;
